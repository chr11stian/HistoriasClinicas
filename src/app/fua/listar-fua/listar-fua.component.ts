...
export class ListarFuaComponent implements OnInit {
    ...
    consultaFUA: string =
        ...
        "/jasperserver/rest_v2/reports/Reports/v1/fuaid/anexo1.pdf?authorization=";
    consultaID: string =
        environment.base_urlTx +
        "/jasperserver/rest_v2/reports/Reports/v1/fuaconsulta/fua_por_consulta.pdf?authorization=";
    listDataFUA: FUA[] = [];
    listDataFuaAux: FUA[] = [];
    linkPDF: string;
    idConsulta: string;
    ...
    constructor(
        ...
        auxData == null
            ? (this.data.estadoAtencion = 2)
            : (this.data.estadoAtencion = auxData.extras.estadoConsulta);
        ...
        this.fuaService
            .getCrearRecuperarFUAxIdConsulta(this.idConsulta)
            .subscribe((res: any) => {
                if (res.cod == "2004") {
                    this.location.back();
                    Swal.fire({
