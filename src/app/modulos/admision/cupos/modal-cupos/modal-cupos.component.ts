...
export class ModalCuposComponent implements OnInit {
  /**Datos de una iprres**/
  idIpressLapostaMedica = JSON.parse(localStorage.getItem("usuario")).ipress
    .idIpress;
  ...
  listTipoConsulta: tipoConsulta[] = [
    {
      name: "CONSULTA GESTANTE",
      value: "CONSULTA_GESTANTE",
      ups: "OBSTETRICIA",
    },
    {
      name: "CONSULTA GESTANTE EXTERNA",
      value: "CONSULTA_GESTANTE_EXTERNA",
      ups: "OBSTETRICIA",
    },
  ...
  changeServicioSelected() {
    ...
    let data = {
      ...
      fechaOferta: this.datePipe.transform(
        this.formCuposOferta.value.fechaOferta,
        "yyyy-MM-dd"
      ),
    };
  ...
  onRowSelect(event) {
    ...
    this.cuposService.PersonalResponsableSeleccionado =
      event.data.personal.nombre; //Personal
    ...
    this.cuposService.tipoConsulta =
      this.formCuposOferta.value.tipoConsulta == ""
        ? this.formCuposOferta.value.SelectUPSOferta
        : this.formCuposOferta.value.tipoConsulta;
  ...
  aceptarDialogCupos() {
    if (this.selectedHorario === undefined) {
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Debe seleccionar el personal de salud",
