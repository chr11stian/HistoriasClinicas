...
export class DiagnosticoConsultaComponent implements OnInit {
      ...
      (res: any) => {
        ...
        if (res.object.denominacion == "ANIOS") {
          if (
            ...
            this.listaDeCIESIS = res.object.diagnostico.filter(
              (element) => element.estado == "ACTIVADO"
            );
          } else {
  ...
  selectedDxHIS(event: any) {
    ...
    console.log(
      "evento desde diagnos ",
      this.formDiagnostico.value.buscarDxHIS
    );
  ...
  getDatatoSaveDx() {
    ...
    var duplicado: boolean = this.diagnosticos.some(
      (element) => element.diagnosticoHIS == aux.diagnosticoHIS
    );
    ...
    if (!duplicado) {
      ...
      this.messageService.add({
        severity: "error",
        summary: "Cuidado!",
        detail: "Ya ingreso este diagnóstico, vuelva a intentar.",
      });
    }
  }
  ...
  editarDx(rowData, rowindex) {
    ...
    this.formDiagnostico
      .get("factorCondicional")
      .setValue(rowData.factorCondicional);
    this.PrestacionService.getDiagnosticoPorCodigo(
      ...
      this.formDiagnostico.patchValue({
        cie10SIS: this.listaDeCIESIS.find(
          (elemento) => elemento.cie10 == rowData.cie10SIS
        ),
      });
    });
    this.cieService
      .getCIEByDescripcion(rowData.cie10HIS)
      .subscribe((res: any) => {
        this.listaDeCIEHIS = res.object;
        this.formDiagnostico.patchValue({
          cie10HIS: this.listaDeCIEHIS.find(
            (elemento) => elemento.codigoItem == rowData.cie10HIS
          ),
        });
      });
