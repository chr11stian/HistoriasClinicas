...
export class SuplementacionCredComponent implements OnInit {
    ...
    constructor(
        private servicio: SuplementacionesMicronutrientesService,
    ...
    getLista() {
        this.servicio
            .getListaMicronutrientes(this.dni)
            .toPromise()
            .then((result) => {
        ...
        this.servicio
            .getListaSuplementacionAnemia(this.dni)
            .toPromise()
            .then((result) => {
    ...
    transformSA() {
        this.suplementacionTerapeutica.forEach((element) => {
            element.fechaTentativa = new Date(
                `${element.fechaTentativa} 00:00:00`
            );
            element.fecha =
                element.fecha != null
                    ? new Date(`${element.fecha} 00:00:00`)
                    : null;
        });
    }
    ...
    separacion() {
        this.SF = this.listaMicronutrientes.filter(
            (item) => item.nombre === "SF"
        );
        // console.log('lista SF', this.SF);
        this.MNM = this.listaMicronutrientes.filter(
            (item) => item.nombre === "MNM"
        );
    ...
    agregarSuplementacion(suplementacion: SuplementacionMicronutrientes) {
        const ref = this.dialogService.open(SuplementoComponent, {
            data: {
                isSuplementacion: this.isSuplementacion,
