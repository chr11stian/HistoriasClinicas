...
export class LoginRolComponent implements OnInit {
  ...
  ingresarLogin() {
    let rol =
      this.formRol.value.rol === "" ? this.roles[0] : this.formRol.value.rol;
      ...
      (res: any) => {
        ...
        if (/*res.object != null &&*/ this.findRol(rol) === "ROLE_ENF_PERSONAL") {
            ...
            if (aux.nombreUPS == "ATENCION INTEGRAL DEL NINO") {
              ...
              this.router.navigate(["/dashboard/cred/citas"]).then(() => {
                window.location.reload();
              });
            } else if (aux.nombreUPS == "OBSTETRICIA") {
              //aux.nombreFuncion == "SERVICIOS ADMINISTRACION" &&
              this.router
                .navigate(["/dashboard/obstetricia-general/citas"])
                .then(() => {
                  window.location.reload();
                });
              ...
              //aux.nombreFuncion == "SERVICIOS ADMINISTRACION" &&
              this.router
                .navigate(["dashboard/consulta-generica/lista-cita/PSICOLOGIA"])
                .then(() => {
                  window.location.reload();
                });
              ...
              //aux.nombreFuncion == "SERVICIOS ADMINISTRACION" &&
              this.router
                .navigate(["dashboard/consulta-generica/lista-cita/NUTRICION"])
                .then(() => {
                  window.location.reload();
                });
            }
