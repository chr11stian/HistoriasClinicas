...
export class ModalAtencionesComponent implements OnInit {
  ...
  llenarCamposTratamientoInmunizaciones() {
    ...
    this.formAtenciones.get("fechaAtencion").setValue(configuracion.fechaAtencion ? configuracion.fechaAtencion : "");
    ...
    if (configuracion.t !== null) { this.formAtenciones.get("temperatura").setValue(configuracion.t); }
    ...
    if (configuracion.fc !== null) { this.formAtenciones.get("pulso").setValue(configuracion.fc); }
    ...
    if (configuracion.acidoFolico !== null) {
      this.formAtenciones.get("acidoFolico").setValue(configuracion.acidoFolico.cantidad + "(" + configuracion.acidoFolico.dosis + ")");
    }
    if (configuracion.hierroYAcidoFolico !== null) {
      this.formAtenciones.get("hierroYAcidoFolico").setValue(configuracion.hierroYAcidoFolico.cantidad + "(" + configuracion.hierroYAcidoFolico.dosis + ")");
    }
    if (configuracion.calcio !== null) {
      this.formAtenciones.get("calcio").setValue(configuracion.calcio.cantidad + "(" + configuracion.calcio.dosis + ")");
    }
    if (configuracion.planParto !== null) {
      this.formAtenciones.get("planPartoReenfocada").setValue(configuracion.planPartoReenfocada);
    }
    if (configuracion.visitaDomiciliaria) {
      if (configuracion.visitaDomiciliaria.estado === "NO" || configuracion.visitaDomiciliaria.estado === "NO APLICA") {
        this.formAtenciones.get("visitaDomiciliaria").setValue(configuracion.visitaDomiciliaria.estado);
      }
      else {
    ...
    if (configuracion.proxCita !== null) { this.formAtenciones.get("proxCita").setValue(configuracion.proxCita); }
    if (configuracion.profesionalACargo !== null) { this.formAtenciones.get("encargado").setValue(configuracion.profesionalACargo.tipoDoc + "-" + configuracion.profesionalACargo.nroDoc); }
